...
function getWeekRangeLabel(days: Date[]): string {
  ...
  return `${formatShortDate(days[0])} - ${days[
    days.length - 1
  ].toLocaleDateString([], {
    month: "short",
    day: "numeric",
...
export function CallendarDashboardClient() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 text-[color:var(--dashboard-text)]">
      {/* <section className="grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_repeat(3,minmax(0,0.32fr))]">
        <Card className={cn(panelClassName, "xl:col-span-1")}>
          <CardHeader className="border-b border-[color:var(--dashboard-border)]">
            <Badge className={cn("w-fit", calendarBadgeClassName)}>
              Consultation Calendar
            </Badge>
            <CardTitle className="mt-3 text-2xl font-semibold tracking-tight text-[color:var(--dashboard-text)] md:text-3xl">
              Simple scheduling, redesigned around the week.
            </CardTitle>
            <CardDescription className="max-w-2xl text-[color:var(--dashboard-text-soft)]">
        ...
        <Card className={cn(panelClassName, "justify-center")}>
          <CardContent className="flex h-full flex-col justify-center py-5">
            <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--dashboard-text-soft)]">
              Pending
            </p>
            <p className="mt-2 text-3xl font-semibold text-[color:var(--dashboard-text)]">
              {metrics.pending}
            </p>
            <p className="mt-1 text-sm text-[color:var(--dashboard-text-soft)]">
      ...
      <div className="grid min-h-full gap-3 xl:grid-cols-[280px_minmax(0,1fr)]">
        <div className="space-y-3 ">
          <Card>
            <CardContent className={"p-0"}>
              <Calendar
                ...
                modifiersClassNames={{
                  hasEvent:
                    "[&_button]:relative [&_button]:after:absolute [&_button]:after:bottom-1.5 [&_button]:after:left-1/2 [&_button]:after:size-1.5 [&_button]:after:-translate-x-1/2 [&_button]:after:rounded-full [&_button]:after:bg-zinc-500",
                }}
              />
            </CardContent>
          </Card>
          ...
          <Card className={panelClassName}>
            <CardHeader>
              <CardTitle className="text-base text-[color:var(--dashboard-text)]">
                Status breakdown
              </CardTitle>
            ...
            <CardContent className="space-y-4 pt-4">
              {[
                ...
                const width =
                  metrics.total === 0
                    ? 0
                    : Math.max(2, (item.count / metrics.total) * 100);

                return (
                  <div key={item.label} className="space-y-1.5">
                    <div className="flex items-center justify-between gap-3 text-sm">
                      <span className="text-[color:var(--dashboard-text-muted)]">
                        {item.label}
                      </span>
        ...
        >
          ...
          <TabsContent value="week" className="min-h-0 flex-1 space-y-0">
                        ...
                        {weekColumns.map((column) => {
                          ...
                          return (
                            <Button
                              ...
                              onClick={() => setSelectedDate(column.day)}
                              className={cn(
                                "h-auto min-w-22 flex-col items-start rounded-[1.25rem] border px-3 py-2 text-left transition",
                                isSelected
                                  ? "border-(--dashboard-accent) bg-(--dashboard-selected) text-(--dashboard-text) hover:bg-(--dashboard-selected)"
                                  : "border-(--dashboard-border) bg-(--dashboard-panel-soft) text-(--dashboard-text-muted) hover:bg-(--dashboard-hover) hover:text-(--dashboard-text)",
                              )}
                            >
                              <p className="text-xs uppercase tracking-[0.16em]">
                                {column.day.toLocaleDateString([], {
                      ...
                      <div className="mt-4 grid grid-cols-[88px_repeat(7,minmax(0,1fr))] gap-3">
                        ...
                        >
                          {timeline.labels.map((hour) => (
                            <div
                              key={hour}
                              className="absolute left-0 text-xs text-[color:var(--dashboard-text-soft)]"
                              style={{
                                top:
                            ...
                            >
                              {new Date(2025, 0, 1, hour, 0).toLocaleTimeString(
                                [],
                                {
                                  hour: "numeric",
                                  minute: "2-digit",
                        ...
                        {weekColumns.map((column) => (
                          <div
                            key={column.dateKey}
                            className="relative rounded-[1.5rem] border border-[color:var(--dashboard-border)] bg-[color:var(--dashboard-panel-soft)]"
                            style={{ height: timeline.height }}
                          >
                            {timeline.labels.map((hour) => (
                              <div
                                key={`${column.dateKey}-${hour}`}
                                className="absolute inset-x-0 border-t border-[color:var(--dashboard-border)]"
                                style={{
                                  top:
                                    (hour - timeline.startHour) *
                                    HOUR_ROW_HEIGHT,
                            ...
                            {column.events.length === 0 ? (
                              <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 rounded-2xl border border-dashed border-[color:var(--dashboard-border)] bg-[color:var(--dashboard-panel)] px-3 py-4 text-center text-xs text-[color:var(--dashboard-text-soft)]">
                                Open schedule
                              </div>
                            ) : null}
                            ...
                            {column.events.map((consultation) => {
                              ...
                              return (
                                <Button
                                  ...
                                  className={cn(
                                    "absolute left-0 right-0 h-auto justify-start border p-0 rounded-sm text-left shadow-[0_18px_40px_-28px_rgba(0,0,0,0.95)] transition hover:scale-[1.01]",
                                    tone.soft,
                                    selectedConsultation?.id === consultation.id
                                      ? "ring-2 ring-[color:var(--dashboard-accent)]/40"
                                      : "",
                                  )}
                                  style={{
                                    top:
                                ...
                                >
                                  <p className="line-clamp-2 text-sm font-semibold text-[color:var(--dashboard-text)]">
                                  ...
                                  <p className="mt-2 line-clamp-2 text-[11px] text-[color:var(--dashboard-text-soft)]">
                ...
                <Card className={panelClassName}>
                  ...
                  <CardContent className="space-y-3 pt-4">
                    {selectedDateEvents.length === 0 ? (
                      <p className="rounded-[1.25rem] border border-dashed border-white/10 bg-white/[0.03] px-3 py-4 text-sm text-[color:var(--dashboard-text-soft)]">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <p className="truncate font-medium text-[color:var(--dashboard-text)]">
                                  {consultation.attendeeName}
                                </p>
          ...
          <TabsContent value="day">
            <Card className={panelClassName}>
              <CardHeader className="border-b border-white/10">
                <CardTitle className="flex items-center gap-2 text-base text-[color:var(--dashboard-text)]">
                  <CalendarClockIcon className="size-4 text-[color:var(--dashboard-text-muted)]" />
                  Daily agenda
                </CardTitle>
                <CardDescription className="text-white/55">
